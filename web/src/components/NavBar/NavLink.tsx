...
const NavLink = ({ name, link, ...rest }: NavLinkProps): JSX.Element => {
    ...
    return pageWidth >= 638 ? (
        ...
        <Link href={link} passHref={true}>
            <a
                className="block pl-3 pr-4 py-2 border-l-4 border-blue-400 text-base font-medium text-gray-200 bg-dark-gray-900 focus:outline-none focus:text-indigo-800 focus:bg-indigo-100 focus:border-indigo-700 transition duration-150 ease-in-out"
                {...rest}
            >
                {name}
            </a>
        ...
        <Link href={link} passHref={true}>
            <a
                className="block pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-gray-300 hover:bg-gray-900 hover:border-gray-300 focus:outline-none focus:text-gray-800 focus:bg-gray-50 focus:border-gray-300 transition duration-150 ease-in-out"
                {...rest}
            >
                {name}
            </a>
